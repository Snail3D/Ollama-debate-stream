...
function handleYouTubeMessage(username, message) {
  ...
  if (message.trim().toLowerCase() === '/clear') {
    if (!isAdmin) {
      console.log(`❌ Non-admin user ${username} tried to use /clear command`);
      return;
    }

    const clearedCount = debateState.queue.length;
    debateState.queue = []; // Clear normal queue
    saveDebateState();

    console.log(`🧹 ADMIN ${username} cleared ${clearedCount} items from normal queue. SuperChat queue protected.`);
    postBotMessage(`🧹 Admin ${username} cleared ${clearedCount} debate(s) from the queue. SuperChat queue remains protected.`);
    return;
  }

  // Handle /remove <number> command (admin only)
  if (message.trim().toLowerCase().startsWith('/remove ')) {
    if (!isAdmin) {
      console.log(`❌ Non-admin user ${username} tried to use /remove command`);
      return;
    }
    ...
    if (isNaN(queueNumber) || queueNumber < 1 || queueNumber > debateState.queue.length) {
      postBotMessage(`❌ Invalid queue number. Please use /remove 1-${debateState.queue.length}`);
      return;
    }
